...
| 方面 | 传统方案 | 本项目方案 | 说明 |
|---|---|---|---|
| **决策逻辑** | 分段硬切换（平移/低速/高速各一套逻辑） | 统一轨迹视角 + 参数线性插值 | 参数更少，更易调试 |
| **弹道解算** | 直接计算，不迭代 | 定点迭代修正，最多 10 次 | 中远距离精度更高 |
| **角速度角度阈值** | 固定 coming/leaving angle | 线性衰减，角速度越高角度越小 | 高转速下缩窄窗口，降低误发 |
| **跳变修正** | 无 | 分方向（pitch_up/pitch_down），时间窗口有效 | 减少跳变瞬间的脱靶 |
| **开火冷却** | 无冷却或固定冷却 | 按角速度动态线性插值冷却时间 | 低速延迟短，高速延迟长 |
| **前哨站特化** | 无 | 独立 coming/leaving angle 和固定冷却时间 | 前哨站角速度恒定，无需动态参数 |
| **前哨站高度建模** | 统一高度或固定偏移 | 运行时自动标定三块装甲板高度偏移 | 解决三块板高度不等问题，提升 pitch 精度 |
| **轨迹规划** | 无（直接跟随目标点） | MPC 规划器（TinyMPC，< 1ms） | 平滑过渡，减少超调 |
| **EKF 健壮性** | 简单 KF，无异常检测 | NIS/NEES 卡方检验 + 滑动窗口统计 | 可监控滤波器发散状态 |
