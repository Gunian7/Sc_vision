...
本项目为杭州电子科技大学Phoenix战队26赛季视觉自瞄模块，不依赖ROS，在参考多支队伍的开源后进行了重构和改进（缝合怪），同时添加了IMM对旋转状态进行单独建模，提升了对变速小陀螺的适应性，测试中使用6mm镜头近距离(4m)内能保持较好的跟踪和击打性能，通过有限状态机进行火控，实际测试中基本2转每秒至少40%以上的命中率（保守估计）。
...
## TODO
1. 职责分层，planner拆分，让其只负责决策，然后aimer和shooter分别负责云台控制和开火控制，解耦决策和执行，两个解耦，以及最终只有一个给C板发的消息，现在这两个冲突了（plan复写aimer）
2. 开火判定重构，或许可以改成根据“几何+散布”阈值（根据实际情况决定）
3. 针对变高变速旋转小陀螺，可能需要给追踪器本身添加一个维度
4. 大小符模型重新训练（可以试试yolov26）
5. 
